...
import 'package:yesorno/tarih_screens/tarih_question_model.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:yesorno/tarih_screens/tarih_result.dart';

class TarihQuestionPage extends StatefulWidget {
  final List<QuestionResult> _questions =
      QuestionResult.getRandomDefaultQuestionsShuffled();

  TarihQuestionPage({super.key});

  @override
  State<StatefulWidget> createState() {
    return _TarihQuestionPageState();
  }
}

class _TarihQuestionPageState extends State<TarihQuestionPage> {
  //in ms
      ...
      body: Container(
        decoration: BoxDecoration(
          image: DecorationImage(
              image: const AssetImage('assets/images/tarih.jpg'),
  ...
  void _showResult(int correctAnswers) {
    Navigator.of(context).pushReplacement(MaterialPageRoute(
        builder: (context) => TarihResultPage(
  ...
  void _updateBestScoreIfNecessary(int correctAnswers) async {
    await SharedPreferences.getInstance().then((sharedPreferences) {
      var bestScore = sharedPreferences.getInt("tarihScore");

      bestScore ??= 0;

      if (bestScore < correctAnswers) {
        sharedPreferences.setInt("tarihScore", correctAnswers);
      }
    });
  }
}
