...
class AllCategoriesResultPage extends StatefulWidget {
  final int allScore, percent;

  const AllCategoriesResultPage(this.allScore, this.percent, {super.key});

  @override
  State<AllCategoriesResultPage> createState() =>
      _AllCategoriesResultPageState();
}

class _AllCategoriesResultPageState extends State<AllCategoriesResultPage> {
  int? _allScore = 0;
  ...
  void _setScore() async {
    await SharedPreferences.getInstance().then((sharedPreferences) {
      setState(() {
        _allScore = sharedPreferences.getInt("allScore") ?? 0;
      });
    });
  }
