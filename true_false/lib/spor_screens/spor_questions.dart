...
import 'package:yesorno/spor_screens/spor_result.dart';
import 'spor_question_model.dart';
import 'package:shared_preferences/shared_preferences.dart';

class SporQuestionPage extends StatefulWidget {
  final List<QuestionResult> _questions =
      QuestionResult.getRandomDefaultQuestionsShuffled();

  SporQuestionPage({super.key});

  @override
  State<StatefulWidget> createState() {
    return _SporQuestionPageState();
  }
}

class _SporQuestionPageState extends State<SporQuestionPage> {
  //in ms
      ...
      body: Container(
        decoration: BoxDecoration(
          image: DecorationImage(
              image: const AssetImage('assets/images/spor.jpeg'),
  ...
  void _showResult(int correctAnswers) {
    Navigator.of(context).pushReplacement(MaterialPageRoute(
        builder: (context) => SporResultPage(
  ...
  void _updateBestScoreIfNecessary(int correctAnswers) async {
    await SharedPreferences.getInstance().then((sharedPreferences) {
      var bestScore = sharedPreferences.getInt("sporScore");

      bestScore ??= 0;

      if (bestScore < correctAnswers) {
        sharedPreferences.setInt("sporScore", correctAnswers);
      }
    });
  }
}
