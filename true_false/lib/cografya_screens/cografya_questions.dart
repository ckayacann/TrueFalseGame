...
import 'package:yesorno/cografya_screens/cografya_result.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'cografya_question_model.dart';

class CografyaQuestionPage extends StatefulWidget {
  final List<QuestionResult> _questions =
      QuestionResult.getRandomDefaultQuestionsShuffled();

  CografyaQuestionPage({super.key});

  @override
  State<StatefulWidget> createState() {
    return _CografyaQuestionPageState();
  }
}

class _CografyaQuestionPageState extends State<CografyaQuestionPage> {
  //in ms
      ...
      body: Container(
        decoration: BoxDecoration(
          image: DecorationImage(
              image: const AssetImage("assets/images/cografya.jpeg"),
  ...
  void _showResult(int correctAnswers) {
    Navigator.of(context).pushReplacement(MaterialPageRoute(
        builder: (context) => CografyaResultPage(
  ...
  void _updateBestScoreIfNecessary(int correctAnswers) async {
    await SharedPreferences.getInstance().then((sharedPreferences) {
      var bestScore = sharedPreferences.getInt("cografyaScore");

      bestScore ??= 0;

      if (bestScore < correctAnswers) {
        sharedPreferences.setInt("cografyaScore", correctAnswers);
      }
    });
  }
}
